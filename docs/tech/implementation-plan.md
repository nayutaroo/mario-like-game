...
- [ ] `package.json` / `tsconfig.json` / `vite.config.ts` / `biome.json` / `index.html` の生成
- [ ] `src/main.ts` で Kaboom（KAPLAY）を初期化、`title` シーン → `play` シーン → `gameover` シーンの遷移を実装
- [ ] `play` シーンに矩形プレイヤー、矩形地面、画面右端の「ゴール矩形」だけ配置。ゴール接触で `gameover` へ
- [ ] GitHub Actions ワークフロー作成、`main` push で GitHub Pages にデプロイ
- [ ] `npm audit` がクリーン、`npm run typecheck` が通る
...
- [ ] 左右移動（歩き / ダッシュ）
- [ ] 可変高度ジャンプ
- [ ] 地形ブロック（壊せない・壊せる）との AABB コリジョン
- [ ] 落下死（Y > 画面外）でリスポーン
- [ ] カメラの横スクロール追従、左方向巻き戻しなし
- [ ] プロトタイプ用ドット絵（待機 / 歩行 / ジャンプ）に差し替え
...
- [ ] イモムシ（直進 + 壁反転、踏みで死亡、横接触でダメージ）
- [ ] ダンゴムシ（踏みで丸まり、再蹴りで弾丸転がり）
- [ ] コバチ（飛行直進、踏みで死亡）
- [ ] プレイヤー被ダメージ → 状態 1 段下げ + 短時間無敵点滅
- [ ] ノーマル状態で被ダメージ → 死亡 → リスポーン
