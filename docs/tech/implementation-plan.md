...
| M3 アイテムと状態機械 | ✅ 完了 | 2026-05-09 |
| M4 レベル拡充と中間ポイント | ✅ 完了 | 2026-05-09 |
| M5 ボス面 | ✅ 完了 | 2026-05-09 |
| M6 隠しエリア・ボーナス面 | ✅ 完了 | 2026-05-09 |
| M7 仕上げ | 〜 進行中 | — |
...
- ダッシュで敵に横からぶつかると踏み判定が暴発する → 単純な `pos.y` 比較から `STOMP_HEIGHT_MARGIN`（12 px）を要求する判定に変更。プレイヤーの底辺が敵の底辺より 12 px 以上上に居る場合のみ stomp として扱う
- ノーマル被ダメージの即死＋即リスポーンが地味 → `dyingTimer`（0.9 秒）を導入し、死亡時は赤く染めて垂直に跳ね上がる演出を挟む。タイマー終了で `reset()` を呼ぶ。演出中は `isLocked()` で入力／敵接触／アイテム取得を全停止
- 残機が減って復帰した時に倒した敵が復活しない → `play.ts` に `spawnEnemies()` / `respawnEnemies()` を追加、`onReset` コールバック内でレベルデータから再生成（アイテムは消費型扱いのため対象外）
- 種の発射方向が見た目で分からない → プレイヤー子オブジェクトとしてくちばし状マーカー（5×8 px）を矩形外側上半分に配置、`facing` の左右切替に追従
...
- [x] ドングリ取得 → どんぐり状態
- [x] リンゴ取得 → リンゴ状態 + 種発射（Z キー）
- [x] 葉っぱ取得 → 一定時間ふわふわ落下（落下速度上限 120 px/s）
- [x] ベリー取得 → スコア+10、100 でライフ+1
- [x] きらきら花取得 → 一定時間無敵（点滅 + 接触で敵を倒せる、8 秒）
- [x] 金のヒナ取得 → ライフ+1
- [x] HUD（スコア・ベリー数・残機・制限時間）

**DoD**: 各アイテムが意図通り動き、HUD が正しく更新される。リンゴ状態で種を撃ってイモムシを倒せる。

**補足**:
- 並列実装: アイテム 6 種 + 種弾は worktree 隔離エージェントで実装、HUD も別エージェントで実装、私は状態機械（`pyo-state.ts`）と player 統合・play.ts 整合を担当
- アイテムの `kind` プロパティは kaplay の component-as-object パターンで持たせ、player 側は `(item as { kind?: ItemKind }).kind` で読み取り
- 種弾のヒット判定は `k.onCollide("seed", "enemy", ...)` のグローバル 2-tag 形式（kaplay の per-obj onCollide 型に typo があるため）
- 制限時間カウントダウンとタイムアップ時のミス処理を play.ts に追加（M4 でステージ遷移と組み合わせて再整理予定）

### M4 — レベル拡充と中間ポイント ✅

**目標**: 1-1〜1-4 の通常面が遊べる。

- [x] レベルデータ形式の確定（TypeScript の型定義 + 各ステージ別ファイル）
- [x] 1-1 草原 / 1-2 森 / 1-3 洞窟 / 1-4 空（縦スクロール採用面）
- [x] 各面に光の鳥居（中間ポイント）配置とリスポーン処理
- [x] 各面のゴール（光の輪）配置とクリア処理 → 次面へ遷移
- [x] 制限時間カウントダウン → 0 でミス（M3 で導入、M4 でチェックポイント連携）
- [x] スコア・残機・ベリーをステージ間で持ち越し

**DoD**: 1-1 → 1-4 を順に通せ、ミス時は中間ポイント以降からリスポーンする。

**補足**:
- レベルデータは `src/levels/stage-1-{1,2,3,4}.ts` の TypeScript 定数として保持。`LevelData` 型は `src/levels/types.ts`、登録は `src/levels/index.ts`
- `src/entities/checkpoint.ts` (光の鳥居 ⛩️) と `src/entities/goal.ts` (光の輪 🌀) を切り出し、play.ts はデータ駆動で配置
- リスポーン位置: `currentCheckpoint` を play.ts が保持、player の `getRespawnPos` コールバックで参照。チェックポイント未到達ならスポーン地点
- 縦スクロール (1-4): `LevelData.verticalScroll: true` のとき camera Y もプレイヤーに追従。X は従来通り「巻き戻さず」、Y は両方向追従
- 死亡演出: 0.9s の dyingTimer (赤く点滅 + ポップアップ) の後にリスポーン（onLifeLost → ライフ判定 → dyingTimer 終了でカメラもスナップ）
...
- [x] ボス「ヌシイモ」エンティティ実装（HP 3、walking → jumping → landing-stun → walking のループ + hurt / dead）
- [x] 踏みで HP -1、種 5 発で HP -1（種は seed-hit カウンタ、5 で 1 ダメージ換算）
- [x] HP 0 で撃破演出 → クリア画面（1.5 秒の余韻付き）
- [x] ボス HP HUD 追加（❤️🖤 表示、撃破で "BOSS DEFEATED!"）

**DoD**: 1-5 城ステージでボスを撃破するとクリア画面が表示される。

**補足**:
- ボス絵文字: 🪲 (beetle) — 従来の敵 (🐛/🐞/🐝) と差別化
- ステージ 1-5 はワンスクリーンのアリーナ（width 1280、両端に壁ブロック）。プレイヤー左寄り、ボス中央、左右に高台 (y=480) を配置して種ヒット狙いやすくしてある
- ボス死亡時の挙動: `untag("enemy", "boss")` で衝突タグを外す → プレイヤーが二度踏みできない、踏んでバウンドもしない。フェードしながら落下
- 撃破イベントは `obj.trigger("defeated")` でブロードキャスト、play.ts が購読してクリア遷移
- スコア: ボス撃破で +1000
- 1-4 の `nextStage` を `"1-5"` に更新済み

### M6 — 隠しエリア・ボーナス面 ✅

**目標**: 探索要素が動く。

- [x] 隠しブロック (? ブロック) の配置（1-1 / 1-2 / 1-3 / 1-4 にそれぞれ 2 個ずつ）
- [x] 隠しエリアからボーナス面へのワープ（1-1 / 1-3 の ? ブロックがワープを内包）
- [x] ボーナス面「木の実の部屋」の実装（敵なし、ベリー大量＋金のヒナ 2 体）
- [x] ボーナス面終了 → 元ステージの次の通常面へ復帰（`PlayOpt.returnTo` で実装）

**DoD**: 1-1 や 1-3 の ? ブロックを下から叩くと 💫 ワープが出現、それに触れるとボーナス面 → ゴールで `returnTo` 先 (1-2 または 1-4) へ戻る。

**補足**:
- マリオ風 ? ブロック (`src/entities/mystery-block.ts`) を新規実装。固体ブロックとして地形扱い、下から叩いた時のみ中身が出現
- 中身は単体アイテム or ボーナス面ワープ。叩くと灰色になり再発動しない
- ワープは標準アイテムと同じ位置に出現させる方式（プレイヤーが取りに行く形）。これによりワープがあるとプレイヤーが少しの間「これを取るかどうか」考える余地が生まれる
- ボーナス面 `bonus-1` (`src/levels/stage-bonus-1.ts`) は単画面 (1280×720)、両端に壁、5 段の浮島構成。ベリー 14 個 + 金のヒナ 2 体
- ボーナス面のゴール接触で `opt.returnTo` を優先して遷移、無ければ通常の `level.nextStage` フローに従う
- `LevelData` に `mysteryBlocks?: MysteryBlockDef[]` と `warps?: WarpDef[]` を追加（warps は単独配置用、現状は ? ブロック内蔵が主）
- 仕様: `docs/specification/mechanics.md` の「? ブロック」セクション参照
...
| 項目 | 値 | 単位 | 備考 |
| --- | --- | --- | --- |
| 重力 | 2400 | px/s² | Kaboom の `setGravity`。滞空時間短縮のため 1800 から増加 |
| 歩き最高速度 | 180 | px/s |  |
| ダッシュ最高速度 | 320 | px/s | Shift 押下時 |
| 移動加速度 | 1200 | px/s² |  |
| 移動減速度（地上） | 1500 | px/s² | 摩擦 |
| 移動減速度（空中） | 600 | px/s² | 慣性強め |
| ジャンプ初速 | 950 | px/s | 押し続けで滞空時間延長。最高点 約 188 px、ピーク到達 0.40 s |
